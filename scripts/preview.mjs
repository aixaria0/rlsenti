...
import { existsSync, mkdirSync, openSync, readFileSync, readdirSync, readlinkSync, rmSync, writeFileSync } from "node:fs";
...
const STATE_DIR = join(ROOT, ".rlsenti");
...
export function parsePreviewArgs(argv) {
  ...
  if (!["stop", "restart"].includes(action)) return { error: `unknown action: ${action} (expected stop or restart)` };
  return { action };
}
export function parsePid(text) { const pid = Number.parseInt(String(text ?? "").trim(), 10); return Number.isInteger(pid) && pid > 1 ? pid : null; }
...
export function looksLikePreviewProcess(cmdline) {
  const argv = String(cmdline ?? "").split("\0").filter(Boolean).join(" ");
...
export function previewOwners({ portPids, pidFilePid, cmdlineOf }) {
  ...
  if (pidFilePid !== null && !owners.has(pidFilePid) && looksLikePreviewProcess(cmdlineOf(pidFilePid))) owners.add(pidFilePid);
  return [...owners];
}
...
export async function terminatePids(pids, { kill, isAlive, sleep, graceMs = GRACE_MS, pollMs = POLL_MS }) {
...
export function stopOutcome({ signalled, stubborn, after }) {
  ...
  if (held.length > 0) return { ok: false, error: `port ${PREVIEW_PORT} is still held by pid(s) ${held.join(", ")}` };
  if (after.unattributed) return { ok: false, error: `port ${PREVIEW_PORT} is held by a process this script cannot see` };
  const message = signalled.length > 0 ? `stopped pid(s) ${signalled.join(", ")} — port ${PREVIEW_PORT} is free` : `nothing was listening on ${PREVIEW_PORT}`;
  return { ok: true, message };
}
const sleep = (ms) => new Promise((resolve) => setTimeout(resolve, ms));
function isAlive(pid) { try { process.kill(pid, 0); return true; } catch (err) { return err?.code === "EPERM"; } }
function pgidOf(pid) { try { return parsePgid(readFileSync(`/proc/${pid}/stat`, "utf8")); } catch { return null; } }
function killPid(pid, signal) {
  if (pgidOf(pid) === pid) { try { process.kill(-pid, signal); return; } catch {} }
  try { process.kill(pid, signal); } catch {}
}
function cmdlineOf(pid) { try { return readFileSync(`/proc/${pid}/cmdline`, "utf8"); } catch { return ""; } }
function readPidFile() { try { return parsePid(readFileSync(PID_FILE, "utf8")); } catch { return null; } }
function pidsForSocketInodes(inodes) {
  ...
  for (const entry of readdirSync("/proc")) {
    ...
    for (const fd of fds) {
      try { if (targets.has(readlinkSync(`/proc/${pid}/fd/${fd}`))) { pids.push(pid); break; } } catch {}
    }
  }
  return pids;
}
function portOwners() {
  ...
  for (const file of ["/proc/net/tcp", "/proc/net/tcp6"]) {
    let dump;
    try { dump = readFileSync(file, "utf8"); } catch { continue; }
...
async function stop(announce = true) {
  const owners = previewOwners({ portPids: portOwners().pids, pidFilePid: readPidFile(), cmdlineOf });
  ...
  if (!outcome.ok) { console.error(`[preview] ${outcome.error}`); return false; }
...
async function waitForReady(failure) {
  ...
  while (Date.now() < deadline && failure() === null) {
    try { await fetch(PREVIEW_URL, { signal: AbortSignal.timeout(2000) }); return true; } catch { await sleep(250); }
  }
  return false;
}
async function restart() {
  ...
  const child = spawn("npm", ["run", "preview"], { cwd: ROOT, detached: true, stdio: ["ignore", log, log] });
  child.unref();
  writeFileSync(PID_FILE, `${child.pid}\n`);
  let failure = null;
  child.on("error", (err) => { failure = `npm run preview could not be spawned: ${err.message}`; });
  child.on("exit", (code, signal) => { failure = `npm run preview exited early (${signal ?? `code ${code}`})`; });
  if (!(await waitForReady(() => failure))) {
    const secs = Math.round(READY_TIMEOUT_MS / 1000);
    const why = failure ?? `nothing answered on ${PREVIEW_URL} within ${secs}s — check vite.config.ts preview.port ${PREVIEW_PORT}`;
...
if (process.argv[1] && import.meta.url === pathToFileURL(process.argv[1]).href) {
  ...
  if (args.error) { console.error(`[preview] ${args.error}`); process.exit(1); }
  if (!existsSync("/proc/self")) { console.error("[preview] no /proc — this script only runs in Linux environments with /proc"); process.exit(1); }
